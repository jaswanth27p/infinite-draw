...
export function VirtualizedFileList<T extends { id: string }>({
  ...
  // The toolbar (search box etc.) must render regardless of loading/error/
  // empty state -- without this, a zero-result search or an in-flight
  // fetch would hide the search box entirely, leaving the user with no way
  // to edit or clear their search term from inside the page.
  return (
    <div className="flex flex-col gap-4">
      {(showViewToggle || toolbar) && (
        <div className={cn("flex items-center gap-2", toolbar ? "justify-between" : "justify-end")}>
          {toolbar}
          {showViewToggle && <ViewModeToggle view={storedView} onChange={setView} />}
        </div>
      )}
      {isPending ? (
        <FileGridSkeleton />
